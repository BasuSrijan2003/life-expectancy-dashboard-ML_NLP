...
<style>
  ...
  .metric-card.red    { background: linear-gradient(135deg, #cb2d3e 0%, #ef473a 100%); box-shadow: 0 4px 15px rgba(203,45,62,0.35); }
...
with tab1:
    ...
    if row.empty:
        ...
        radar_fig.add_trace(go.Scatterpolar(
            r=country_vals + [country_vals[0]], theta=radar_labels + [radar_labels[0]],
            fill="toself", name=country, line_color="#667eea", fillcolor="rgba(102,126,234,0.2)"
        ))
        radar_fig.add_trace(go.Scatterpolar(
            r=global_vals + [global_vals[0]], theta=radar_labels + [radar_labels[0]],
        ...
        if not country_all.empty:
            ...
            ts_fig.add_vline(x=year, line_dash="dash", line_color="#ef473a",
...
with tab2:
    ...
    if predict_btn:
        ...
        compare_fig.update_layout(barmode="group", height=320,
                                  paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
with tab3:
    ...
    w_country = st.selectbox("Base Country", sorted(raw_df["Country"].dropna().unique()),
                              index=sorted(raw_df["Country"].dropna().unique()).index("India"),
                              key="whatif_country")
    w_year = st.selectbox("Base Year", sorted(raw_df["Year"].dropna().unique()),
                           index=len(years) - 1, key="whatif_year")
    ...
    if base_row.empty:
        ...
        br = base_row.iloc[0]
        status_enc = 1 if br["Status"] == "Developed" else 0
        ...
        if run_sim:
            ...
            mod_inp["Schooling"]  = w_school
            mod_inp["HIV/AIDS"]   = w_hiv
            mod_inp["GDP"]        = w_gdp
            mod_inp["Adult Mortality"] = w_am
            mod_inp["Income composition of resources"] = w_income
            mod_inp["Total expenditure"] = w_totexp
            ...
            lever_keys = {
                "Schooling": "Schooling", "HIV/AIDS Rate": "HIV/AIDS",
                "GDP": "GDP", "Adult Mortality": "Adult Mortality",
                "Income Index": "Income composition of resources",
                "Health Expenditure": "Total expenditure",
            }
            lever_vals = {
                "Schooling": w_school, "HIV/AIDS Rate": w_hiv, "GDP": w_gdp,
                "Adult Mortality": w_am, "Income Index": w_income, "Health Expenditure": w_totexp,
            }
            impacts = []
            for label, key in lever_keys.items():
                single      = base_inp.copy()
                single[key] = lever_vals[label]
                impacts.append({"Lever": label, "Impact (years)": round(predict(single) - base_pred, 3)})
            ...
            imp_fig = go.Figure(go.Bar(
                x=imp_df["Impact (years)"], y=imp_df["Lever"], orientation="h",
                marker_color=colors,
                text=[f"{v:+.2f} yrs" for v in imp_df["Impact (years)"]],
...
with tab4:
    ...
    LABELS = {
        ...
        "BMI":                               "Average BMI",
        "under-five deaths":                 "Under-5 Child Deaths",
        "thinness  1-19 years":              "Child/Teen Thinness Rate",
        "Alcohol":                           "Alcohol Consumption",
        "Year":                              "Year (Temporal Trend)",
        "Total expenditure":                 "Gov. Health Expenditure (% GDP)",
        "Diphtheria":                        "Diphtheria Vaccination %",
        "Polio":                             "Polio Vaccination %",
        "GDP":                               "GDP per Capita",
        "Hepatitis B":                       "Hepatitis B Vaccination %",
        "percentage expenditure":            "Health Expenditure per Capita",
    ...
    imp_fig = go.Figure(go.Bar(
        ...
        marker_color=colors,
        text=[f"{v*100:.2f}%" for v in importances.values],
        textposition="outside",
    ))
    imp_fig.update_layout(
        height=600, margin=dict(l=10, r=80, t=20, b=20),
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(title="Importance Score", gridcolor="#f3f4f6", tickformat=".0%"),
        yaxis=dict(showgrid=False),
    )
    ...
    corr     = clean_df.corr()
    heat_fig = px.imshow(corr, color_continuous_scale="RdBu_r", zmin=-1, zmax=1,
                         aspect="auto", text_auto=".2f")
    heat_fig.update_layout(height=600, paper_bgcolor="rgba(0,0,0,0)",
